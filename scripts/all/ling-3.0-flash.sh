#!/usr/bin/env bash
#
# ling-3.0-flash.sh — per-model training pipeline for Ling-3.0-flash.
#
# Thin wrapper around scripts/run_train_pipeline.sh that pins --config to
# configs/all_models/ling-3.0-flash.yaml so swapping models is a single
...
#   bash scripts/all/ling-3.0-flash.sh
#   bash scripts/all/ling-3.0-flash.sh --ddp 4
#   bash scripts/all/ling-3.0-flash.sh --seed 7
set -euo pipefail
...
export SGLANG_SGLOG="sglang_server_ling-3.0-flash.log"
